...
class JackHammer : public Pattern {
  public:
    ...
    void setSpeedLimit(unsigned int maxSpeed, unsigned int maxAcceleration,
                       unsigned int stepsPerMM) {
        _maxSpeed = maxSpeed;
        _maxAcceleration = maxAcceleration;
        _stepsPerMM = stepsPerMM;
    ...
    motionParameter nextTarget(unsigned int index) {
        ...
        if (index == 0 || _nextMove.stroke >= _depth) {
            ...
            // acceleration to meet the profile
            _nextMove.acceleration =
                int(3.0 * float(_nextMove.speed) / _timeOfStroke);
        ...
        // only calculate new position, if index has incremented: no mid-stroke
        // update, as vibration is sufficiently fast except for return stroke if
        // depth is exceeded.
        if (index != _index) {
            // Vibration happens at maximum speed and acceleration of the
            // machine
            ...
            if (index % 2) {
                ...
                _nextMove.stroke =
                    min((_nextMove.stroke + _inVibrationDistance), _depth);
            }
        }
        _index = index;
        return _nextMove;
  ...
  protected:
    ...
    void _updateVibrationParameters() {
        ...
        /* Calculate _outVibrationDistance to match with stroking speed
           ...
           Formula neglects acceleration. Real timing will be slower due to
           finite acceleration & deceleration
        */
        _outVibrationDistance = _inVibrationDistance *
                                (_maxSpeed - _strokeInSpeed) /
                                (_maxSpeed + _strokeInSpeed);

#ifdef DEBUG_PATTERN
        Serial.println("_maxSpeed: " + String(_maxSpeed) + " _strokeInSpeed: " +
                       String(_strokeInSpeed) + " _strokeOutSpeed: " +
                       String(int(1.5 * _stroke / _timeOfStroke)));
        Serial.println("inDist: " + String(_inVibrationDistance) +
                       " outDist: " + String(_outVibrationDistance));
#endif
    }
};
...
class StrokeNibbler : public Pattern {
  public:
    ...
    motionParameter nextTarget(unsigned int index) {
        ...
        // only calculate new position, if index has incremented: no mid-stroke
        // update, as vibration is sufficiently fast
        if (index != _index) {
            if (_returnStroke == true) {
                ...
                if (index % 2) {
                    ...
                    _nextMove.stroke =
                        max(_nextMove.stroke - _inVibrationDistance,
                            _depth - _stroke);
  ...
  protected:
    ...
    int _strokeSpeed = 0;
    void _updateVibrationParameters() {
        // Empirical factor to compensate time losses due to finite
        // acceleration.
        ...
        */
        _outVibrationDistance = _inVibrationDistance *
                                (_maxSpeed - _strokeSpeed) /
                                (_maxSpeed + _strokeSpeed);

#ifdef DEBUG_PATTERN
        Serial.println("_maxSpeed: " + String(_maxSpeed) +
                       " _strokeSpeed: " + String(_strokeSpeed));
        Serial.println("inDist: " + String(_inVibrationDistance) +
                       " outDist: " + String(_outVibrationDistance));
#endif
    }
